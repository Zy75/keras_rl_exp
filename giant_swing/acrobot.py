...
#        high = np.array([4.0, 0.7, 2.0, 2.0])
        high = np.array([0.1, 0.1, 0.1, 0.1])
        self.state = self.np_random.uniform(low=-high, high=high)
 
        return self._get_ob()

    def get_rewardAA(self,th1,th2,dth1):
        
        l1 = self.LINK_LENGTH_1
        l2 = self.LINK_LENGTH_2

        tip_angle = th1 + math.atan2( l2 * math.sin( th2 ) , l1 + l2 * math.cos( th2 ) ) 

        return 2.0 * math.fabs( tip_angle ) - math.fabs( th1 )

    def l2cheight(self, th1, th2):
  
        l1 = self.LINK_LENGTH_1
        l2c = self.LINK_COM_POS_2

        return - l1 * cos( th1 ) - l2c * cos( th1 + th2 )

    def get_rewardCC(self,th1,th2,dth1):
 

        l2cheight = self.l2cheight(th1,th2)

        self.l2cheight_old = l2cheight
      
        return ( l2cheight - self.l2cheight_old ) / self.dt
...
#        reward +=  3.0 if terminal else 0.0
    ...
    def _terminal(self):
        ...
        if theta1 > 3.0 * pi / 2.0:
           return True
        elif theta1 < - 3.0 * pi / 2.0:
           return True
        else:
           return False
